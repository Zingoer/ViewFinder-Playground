...
navBarView.backgroundColor = UIColor(red:0.42, green:0.13, blue:0.65, alpha:1)
...
// Draw View Finder with translucent navigation bar
let painter = ViewFinderPainter(frame: containerView.frame)
painter.pickMode(.ShowFinderInParentViewCenterNonTranslucentNavBar(navBarHeight: navBarHeight+statusBarHeight))
let gray = painter.createGrayMask()
let frame = painter.createFrame()
